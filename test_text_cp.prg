...
lcTempFile = 'chars.txt'
lcChars = ''

* Генерируем тестовые данные
For i = 192 To 191 + 32
  lcChars = lcChars + Textmerge("<<'CODE: #' + TRANSFORM(i) + ' - ' + CHR(i) + ', CODE: #' + TRANSFORM(i+32) + ' - ' + CHR(i+32) + CHR(13) + CHR(10)>>")
Endfor
...
For i=1 To Alen(aCharsets,1)
  ...
  *Проверяем доступность чарсета для шрифта
  ...
  IF !llErr
     ...
     Strtofile( lcChars, lcTmp )
     CreateChild(lcTmp, aCharsets[i,1], aCharsets[i,2], lnj)
  ENDIF
Endfor

Strtofile( lcChars, lcTempFile)
...
Procedure CreateChild(tcFile, tnCharset, tcName, tnIndex)
  ...
  Define Window (lcWin) ;
    FROM 2*tnIndex, 2*tnIndex To 35+2*tnIndex, 60+2*tnIndex ;
    FONT C_FONTNAME, 12 ;
